    ...
    <div className="p-[24px] flex flex-col gap-[24px] w-full relative pb-[100px]">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-[240px] shrink-0 flex flex-col gap-2">
          ...
          <button onClick={() => setActiveTab('theme')} className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-semibold transition-colors ${activeTab === 'theme' ? 'bg-[#E5F0FF] text-[#003F87]' : 'text-slate-600 hover:bg-slate-50'}`}>
        ...
        <div className="flex-1 bg-white border border-[#C2C6D4] rounded-xl shadow-sm p-6 lg:p-8">
          ...
          {activeTab === 'theme' && (
            <form onSubmit={handleSave} className="flex flex-col gap-6 animate-in fade-in">
              <h3 className="text-xl font-bold text-slate-800 border-b border-slate-200 pb-4">Theme & Appearance</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-1.5 md:col-span-2">
                  <label className="text-xs font-bold text-slate-600 uppercase">Global Theme</label>
